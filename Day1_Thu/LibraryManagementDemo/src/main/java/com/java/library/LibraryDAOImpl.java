...
// to search the list of books by their id, name and all -------------------------------------
	@Override
...
// to issue the book and update them in the table by subtracting one book from db -------------------
	@Override
	public String updateBooks(int bookId, String user) throws ClassNotFoundException, SQLException {
		...
		if (hasUserIssuedBook(bookId, user)) {
			return "   Book Id : " +bookId+"	Status : Already Issued !!";
		} else {
			...
			pst.executeUpdate();
			return "   Book Id : " +bookId+"	Status : Issued Successfully";
		}
	}
...
// to get the user -----------------------------------------------------------------------------
	...
	@Override
	public String returnBooks(int bookId, String userName) throws ClassNotFoundException, SQLException {
		/*
		 * connection = ConnectionHelper.getConnection();
		 * 
		 * 
		 * Books book = getBookById(bookId); Books book1 = getBookByUser(user);
		 * 
		 * 
		 * String cmd = "DELETE FROM TranBook WHERE bookId = ? AND userName = ?"; pst =
		 * connection.prepareStatement(cmd); pst.setInt(1, bookId);
		 * 
		 * pst.setString(2, user); pst.executeUpdate();
		 * 
		 * cmd = "Insert into transreturn(bookId,userName) values(?,?)"; pst =
		 * connection.prepareStatement(cmd); pst.setInt(1, bookId); pst.setString(2,
		 * user); pst.executeUpdate();
		 * 
		 * cmd = "UPDATE Books SET TotalBooks = TotalBooks + 1 WHERE Id = ?"; pst =
		 * connection.prepareStatement(cmd); pst.setInt(1, bookId); pst.executeUpdate();
		 */
		connection = ConnectionHelper.getConnection();
		String cmd = "Delete from tranbook where bookId = ?";
		pst = connection.prepareStatement(cmd);
		pst.setInt(1, bookId);
		pst.executeUpdate();
		...
		pst.executeUpdate();
		return "Book Id : "+bookId+ 	"Status : Returned Sucessfully";
	}
